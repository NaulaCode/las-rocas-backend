      ...
      <div className="overflow-x-auto">
        <table className="w-full">
            ...
            <tr className="bg-gray-50 border-b border-gray-100">
              {resolvedHeaders.map((h, i) => (
                <th key={i}
                  ...
                  className={`px-4 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wider ${i === resolvedHeaders.length - 1 ? 'text-center' : 'text-left'} ${h.sortable && onSort ? 'cursor-pointer select-none hover:bg-gray-100/50' : ''}`}
                >
          ...
          <tbody className="divide-y divide-gray-50">
            {rows.length > 0 ? rows : (
              <tr>
                <td colSpan={resolvedHeaders.length} className="px-4 py-12 text-center text-gray-400">
