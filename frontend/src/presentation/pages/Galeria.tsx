                  ...
                  <div className={`absolute inset-0 transition-all duration-500 flex items-center justify-center ${isVideo(entry) ? 'pointer-events-none' : 'bg-black/0 group-hover:bg-black/40'}`}>
                    <div className={`${isVideo(entry) ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'} transition-all duration-500 transform ${isVideo(entry) ? 'scale-100' : 'scale-0 group-hover:scale-100'}`}>
                      {isVideo(entry) ? (
                        <button
                          type="button"
                          className="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center pointer-events-auto cursor-pointer hover:bg-white/30 transition-colors"
                          onClick={(e) => {
                            e.stopPropagation();
                            const video = (e.currentTarget.closest('.relative') as HTMLElement)?.querySelector('video');
                            if (video) { video.paused ? video.play() : video.pause(); }
                          }}
                          aria-label={t('galeria.reproducir')}
                        >
                          <svg className="w-6 h-6 text-white ml-0.5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>
                        </button>
                      ) : (
                        <div className="w-14 h-14 bg-white/20 backdrop-blur-sm rounded-full flex items-center justify-center pointer-events-none">
        ...
        <div className="container mx-auto px-4 text-center">
          ...
          >
            ...
            <div className="flex justify-center gap-3">
              <a href={`https://www.instagram.com/explore/tags/${t('galeria.hashtag').replace('#', '')}/`} target="_blank" rel="noopener noreferrer" className="px-6 py-3 bg-gradient-to-r from-purple-500 via-pink-500 to-orange-400 text-white rounded-full font-semibold hover:scale-105 transition-all text-sm shadow-lg">
